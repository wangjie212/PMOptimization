function [xx, rho]=CSPMOsolve_dual(prob,order)
...
for i=1:length(prob.theta)
    ...
    for j=1:sdegs0
        for k=1:j
            ...
            for w=1:sphi
                index=getindex(degs0(i,:)+degs0(j,:)+degree(t_theta(w),prob.Y));
...
k_x=max(max(d_G,d_x_P),degree(obj));
...
if strcmp(prob.type, 'convex')
 ...
 if flag==1
     ...
     M=M(1:nchoosek(n+t,t), 1:nchoosek(n+t,t));
    [X, W]=extraction(M, n, 1, t);
 end
end
